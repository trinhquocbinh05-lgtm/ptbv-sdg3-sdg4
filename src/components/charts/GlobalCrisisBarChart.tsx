...
export const GlobalCrisisBarChart: React.FC = () => {
  ...
  return (
    <div className="liquid-glass rounded-2xl p-4 sm:p-5 lg:p-6 border border-rose-500/30 shadow-xl">
      <div className="flex items-center justify-between pb-2.5 mb-3 border-b border-white/10">
        <div className="flex items-center gap-2">
          <AlertTriangleIcon className="w-4 h-4 sm:w-5 sm:h-5 text-rose-400" />
          <span className="text-xs sm:text-sm font-mono font-bold text-rose-300 uppercase tracking-wider">
            Biểu đồ Báo động Khủng hoảng Y tế Toàn cầu
          </span>
        </div>
        <span className="text-xs text-white/50 font-mono hidden sm:inline">UN SDG Progress Report 2024</span>
      </div>
      ...
      <div className="space-y-3">
        {bars.map((item) => {
          ...
          return (
            <div key={item.metric} className="p-3 sm:p-3.5 rounded-xl bg-white/[0.03] border border-white/10">
              ...
              {/* Progress visualizer */}
              <div className="h-3 sm:h-3.5 w-full bg-black/60 rounded-full overflow-hidden flex p-0.5 border border-white/10 relative">
                <AnimatedProgressBar
                  width={`${ratio}%`}
                  duration={1.2}
              ...
              <div className="flex justify-between items-center text-xs text-white/70 mt-1.5">
